<div class="relative" x-data="{
    ...
    <button
        ...
        type="button"
    >
        <span class="mr-3 flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-brand-500 text-sm font-semibold text-white">
            {{ $initials !== '' ? $initials : 'PM' }}
        </span>

       <span class="mr-1 block font-medium text-theme-sm">{{ $user?->name }}</span>
    ...
    >
        <div>
            ...
            <span class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">{{ $user?->username }}</span>
            @if ($userRole)
                <span class="mt-2 inline-flex rounded-full bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-700 dark:bg-brand-500/10 dark:text-brand-300">
                    {{ $userRole->name }}
                </span>
            @endif
        </div>
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            <button
                type="submit"
                class="group mt-3 flex w-full items-center gap-3 rounded-lg px-3 py-2 font-medium text-gray-700 text-theme-sm hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300"
                @click="closeDropdown()"
            >
                <span class="text-gray-500 group-hover:text-gray-700 dark:group-hover:text-gray-300">
