    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola add-ons per malam dan per stay untuk kebutuhan booking.</p>
            </div>
        ...
        <x-common.component-card title="Daftar Add-ons" desc="Konfigurasi add-ons berbasis TailAdmin untuk pricing tambahan.">
            <div class="overflow-x-auto custom-scrollbar">
                <table class="min-w-full">
                    <thead><tr class="border-b border-gray-100 dark:border-gray-800"><th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Nama</th><th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Tipe Charge</th><th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Harga</th><th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Status</th><th class="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Aksi</th></tr></thead>
                    <tbody>
                        @forelse ($addons as $addon)
                            <tr class="border-b border-gray-100 last:border-b-0 dark:border-gray-800"><td class="px-5 py-4 text-sm font-medium text-gray-800 dark:text-white/90">{{ $addon->name }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $addon->charge_type === 'per_night' ? 'Per malam' : 'Per stay' }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">Rp {{ number_format($addon->price, 0, ',', '.') }}</td><td class="px-5 py-4">@if($addon->is_active)<x-ui.badge color="success">Aktif</x-ui.badge>@else<x-ui.badge color="light">Nonaktif</x-ui.badge>@endif</td><td class="px-5 py-4"><div class="flex items-center justify-end gap-2"><a href="{{ route('addons.edit', $addon) }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Edit</a><form method="POST" action="{{ route('addons.destroy', $addon) }}" onsubmit="return confirm('Hapus add-on ini?');">@csrf @method('DELETE')<button type="submit" class="rounded-lg border border-error-200 px-4 py-2 text-sm font-medium text-error-600 transition hover:bg-error-50 dark:border-error-800 dark:text-error-400 dark:hover:bg-error-500/10">Hapus</button></form></div></td></tr>
                        @empty
                            <tr><td colspan="5" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada add-on yang dibuat.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
