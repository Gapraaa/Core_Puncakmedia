...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Daftar Booking</h2>
            ...
            <a href="{{ route('bookings.create') }}" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-5 py-3 text-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600">Buat Booking</a>
        </div>

        <x-common.component-card title="Filter Booking" desc="Cari booking berdasarkan kode, tamu, status, brand, dan tanggal check-in.">
            <form method="GET" action="{{ route('bookings.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
                <div class="xl:col-span-2"><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Pencarian</label><input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Kode, invoice, tamu, telepon" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90" /></div>
                <div><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Status Payment</label><select name="payment_status" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90"><option value="">Semua</option><option value="unpaid" @selected(($filters['payment_status'] ?? '') === 'unpaid')>Unpaid</option><option value="partial" @selected(($filters['payment_status'] ?? '') === 'partial')>Partial</option><option value="paid" @selected(($filters['payment_status'] ?? '') === 'paid')>Paid</option></select></div>
                <div><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Status Booking</label><select name="booking_status" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90"><option value="">Semua</option><option value="draft" @selected(($filters['booking_status'] ?? '') === 'draft')>Draft</option><option value="pending_payment" @selected(($filters['booking_status'] ?? '') === 'pending_payment')>Pending Payment</option><option value="confirmed" @selected(($filters['booking_status'] ?? '') === 'confirmed')>Confirmed</option></select></div>
                <div><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Brand</label><select name="brand_id" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90"><option value="">Semua brand</option>@foreach ($brands as $brand)<option value="{{ $brand->id }}" @selected((string) ($filters['brand_id'] ?? '') === (string) $brand->id)>{{ $brand->name }}</option>@endforeach</select></div>
                ...
                <div class="flex items-end gap-3 xl:col-span-6"><button type="submit" class="rounded-lg bg-brand-500 px-5 py-3 text-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600">Terapkan Filter</button><a href="{{ route('bookings.index') }}" class="rounded-lg border border-gray-300 px-5 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Reset</a></div>
            </form>
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Villa</th>
                    ...
                    <tbody>
                        @forelse ($bookings as $booking)
                            <tr class="border-b border-gray-100 last:border-b-0 dark:border-gray-800">
                                ...
                                <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $booking->villa?->name }}<div class="text-xs text-gray-500 dark:text-gray-400">{{ $booking->villaUnit?->unit_name }}</div></td>
                                ...
                                <td class="px-5 py-4"><div class="flex flex-col gap-2"><x-ui.badge :color="$booking->payment_status === 'paid' ? 'success' : ($booking->payment_status === 'partial' ? 'warning' : 'light')">{{ $booking->payment_status }}</x-ui.badge><x-ui.badge :color="$booking->booking_status === 'confirmed' ? 'success' : ($booking->booking_status === 'pending_payment' ? 'warning' : 'light')">{{ $booking->booking_status }}</x-ui.badge></div></td>
