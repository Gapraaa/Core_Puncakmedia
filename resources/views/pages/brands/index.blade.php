...
@section('content')
    ...
    <div class="space-y-6" data-async-page="true">
        ...
        <x-common.component-card title="Filter Brand" desc="Cari brand berdasarkan nama atau slug.">
            <form data-async-page-form="true" method="GET" action="{{ route('brands.index') }}" class="flex flex-col gap-4 md:flex-row md:items-end">
                <div class="w-full md:max-w-md"><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Pencarian</label><input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Nama atau slug" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90" /></div>
                            ...
                            <tr>
                                <td colspan="4" class="px-5 py-10">
                                    <x-common.empty-state
                                        ...
                                        description="Sistem masih fresh. Tambahkan brand dulu supaya villa bisa dikelompokkan dengan rapi sejak awal."
                                        actionLabel="Tambah Brand"
                                        :actionHref="route('brands.create')"
                                    />
                                </td>
                            </tr>
                        @endforelse
