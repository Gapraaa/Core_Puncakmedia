<div class="grid grid-cols-12 gap-4 md:gap-6">
    <div class="col-span-12 xl:col-span-8">
        <x-common.component-card :title="$pageTitle" :desc="$pageDescription">
            <form method="POST" action="{{ $formAction }}" class="space-y-6">
                ...
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Info Bank</label>
                    <textarea name="bank_info" rows="5" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90">{{ old('bank_info', $brand->bank_info) }}</textarea>
                    @error('bank_info')<p class="mt-2 text-sm text-error-600 dark:text-error-400">{{ $message }}</p>@enderror
                </div>
    ...
    <div class="col-span-12 xl:col-span-4">
        <x-common.component-card title="Catatan Brand" desc="Jaga konsistensi data master agar booking dan referensi finance tetap sinkron.">
            <div class="space-y-3 text-sm text-gray-600 dark:text-gray-300">
                ...
                <div class="rounded-xl border border-gray-100 px-4 py-3 dark:border-gray-800">Info bank akan dipakai pada flow payment dan invoice di tahap berikutnya.</div>
