            ...
            <div class="col-span-12 xl:col-span-4">
                <x-common.component-card title="Informasi Invoice" desc="Invoice ini hanya referensi keluar ke tamu. Sumber utama transaksi internal tetap booking induknya.">
                    <div class="space-y-3 text-sm text-gray-600 dark:text-gray-300">
                        ...
                        @endif
                        <div><span class="font-medium text-gray-800 dark:text-white/90">Periode:</span> {{ $invoice->booking?->check_in?->format('j M Y') }} - {{ $invoice->booking?->check_out?->format('j M Y') }}</div>
                ...
                <table class="min-w-full">
                    ...
                    <tbody>
                        @forelse ($invoice->items as $item)
                                                <tr class="border-b border-gray-100 last:border-b-0 dark:border-gray-800"><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $item->item_type }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $item->item_name }}<div class="text-xs text-gray-500 dark:text-gray-400">{{ $item->notes }}</div></td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $item->reference_date?->format('j M Y') ?: '-' }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $item->quantity }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ number_format($item->unit_price, 0, ',', '.') }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ number_format($item->total_price, 0, ',', '.') }}</td></tr>
                        @empty
                ...
                <table class="min-w-full">
                    ...
                    <tbody>
                        @forelse ($invoice->payments as $payment)
                                                <tr class="border-b border-gray-100 last:border-b-0 dark:border-gray-800"><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $payment->paid_at?->format('j M Y H:i') }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ ucfirst($payment->payment_method) }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ ucfirst(str_replace('_', ' ', $payment->received_by)) }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ number_format($payment->amount, 0, ',', '.') }}</td><td class="px-5 py-4 text-right"><a href="{{ route('documents.payments.receipt', ['payment' => $payment, 'download' => 1]) }}" class="text-sm font-medium text-brand-600 dark:text-brand-300">Unduh Bukti</a></td></tr>
                        @empty
