...
@section('content')
    <x-common.page-breadcrumb pageTitle="Pilih Unit Invoice" />
    ...
    <div class="space-y-6" data-async-page="true">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Villa resort ini memiliki {{ $villa->units_count }} unit. Pilih unit terlebih dahulu untuk melihat daftar invoice.</p>
            </div>
            <a href="{{ route('invoices.index') }}" class="inline-flex items-center justify-center rounded-lg border border-gray-300 px-5 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Kembali ke Daftar Villa</a>
        </div>

        <x-common.component-card title="Filter Unit Resort" desc="Cari unit resort berdasarkan nama unit, tipe, dan status.">
            <form data-async-page-form="true" method="GET" action="{{ route('invoices.units', $villa) }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
                <div class="xl:col-span-2"><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Pencarian</label><input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Nama unit atau tipe unit" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90" /></div>
                ...
                <div class="flex gap-3 xl:col-span-3"><button type="submit" class="rounded-lg bg-brand-500 px-5 py-3 text-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600">Terapkan Filter</button><a href="{{ route('invoices.units', $villa) }}" class="rounded-lg border border-gray-300 px-5 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Reset</a></div>
            </form>
        </x-common.component-card>

        <x-common.component-card title="Daftar Unit Resort" desc="Setiap unit memiliki daftar invoice yang dikelola terpisah.">
                    ...
                    <tbody>
                        @forelse ($villaUnits as $villaUnit)
                            <tr class="border-b border-gray-100 last:border-b-0 dark:border-gray-800">
                                ...
                                <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $villaUnit->bookings_count }} booking</td>
                                <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $villaUnit->invoices_count }} invoice</td>
                                <td class="px-5 py-4"><div class="flex items-center justify-end gap-2"><a href="{{ route('invoices.unit', [$villa, $villaUnit]) }}" class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-brand-600">Lihat Invoice</a></div></td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">Villa resort ini belum memiliki unit.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
