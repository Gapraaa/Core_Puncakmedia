...
@section('content')
    ...
    <div class="space-y-6" data-async-page="true">
        ...
        <x-common.component-card title="Filter Pembayaran" desc="Cari pembayaran berdasarkan booking, tamu, metode, dan penerima.">
            <form data-async-page-form="true" method="GET" action="{{ route('payments.unit', [$villa, $villaUnit]) }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div class="xl:col-span-2"><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Pencarian</label><input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Kode booking, tamu, catatan" class="ops-input" /></div>
                        ...
                        @forelse ($payments as $payment)
                            <tr>
                                ...
                                <td>{{ $payment->booking?->guest_name ?? '-' }}</td>
                                        <td>{{ $payment->paid_at?->format('j M Y H:i') }}</td>
