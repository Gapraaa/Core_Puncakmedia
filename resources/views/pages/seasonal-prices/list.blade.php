...
@section('content')
    ...
    <div class="space-y-6" data-async-page="true">
        ...
        <x-common.component-card title="Filter Harga High Season" desc="Cari data berdasarkan catatan override harga untuk unit ini.">
            <form data-async-page-form="true" method="GET" action="{{ $villa->is_resort ? route('seasonal-prices.unit', [$villa, $villaUnit]) : route('seasonal-prices.villa', $villa) }}" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Pencarian</label><input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Catatan high season" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90" /></div>
                ...
                <table class="min-w-full">
                    ...
                    <tbody>
                        @forelse ($seasonalPrices as $seasonalPrice)
                                            <tr class="border-b border-gray-100 last:border-b-0 dark:border-gray-800"><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $seasonalPrice->start_date->format('j M Y') }} - {{ $seasonalPrice->end_date->format('j M Y') }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">Rp {{ number_format($seasonalPrice->price, 0, ',', '.') }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $seasonalPrice->note ?: '-' }}</td><td class="px-5 py-4"><div class="flex items-center justify-end gap-2"><a href="{{ route('seasonal-prices.edit', $seasonalPrice) }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Edit</a><form method="POST" action="{{ route('seasonal-prices.destroy', $seasonalPrice) }}" data-confirm="Hapus harga high season ini?" data-confirm-title="Hapus Harga Seasonal" data-confirm-label="Ya, hapus harga" data-toast-loading="Harga seasonal sedang dihapus." data-toast-loading-title="Menghapus Harga Seasonal">@csrf @method('DELETE')<button type="submit" class="rounded-lg border border-error-200 px-4 py-2 text-sm font-medium text-error-600 transition hover:bg-error-50 dark:border-error-800 dark:text-error-400 dark:hover:bg-error-500/10">Hapus</button></form></div></td></tr>
                        @empty
