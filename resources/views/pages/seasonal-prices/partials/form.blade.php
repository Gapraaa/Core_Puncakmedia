        ...
        <x-common.component-card :title="$pageTitle" :desc="$pageDescription">
            <form method="POST" action="{{ $formAction }}" class="space-y-6"
                ...
                <div>
                    ...
                    @if (!empty($selectedVillaUnit))
                        <input type="hidden" name="villa_unit_id" value="{{ old('villa_unit_id', $selectedVillaUnit->id) }}">
                        <div class="rounded-lg border border-gray-300 bg-gray-50 px-4 py-3 text-sm text-gray-700 dark:border-gray-700 dark:bg-white/[0.03] dark:text-gray-200">
                            {{ $selectedVilla?->name }} - {{ $selectedVillaUnit->unit_name }}
                        </div>
                    @else
                ...
                <div><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Harga High Season (Rupiah)</label><input type="number" step="1" min="0" name="price" value="{{ old('price', $seasonalPrice->price ?? 0) }}" data-money class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90" /></div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ !empty($selectedVillaUnit) && !empty($selectedVilla) ? ($selectedVilla->is_resort ? route('seasonal-prices.unit', [$selectedVilla, $selectedVillaUnit]) : route('seasonal-prices.villa', $selectedVilla)) : route('seasonal-prices.index') }}" class="rounded-lg border border-gray-300 px-5 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Batal</a>
