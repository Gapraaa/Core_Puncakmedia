    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola identitas villa, kapasitas, status operasional, dan konten deskriptif.</p>
            </div>
                ...
                <table class="min-w-full">
                    ...
                    <tbody>
                        @forelse ($villas as $villa)
                            <tr class="border-b border-gray-100 last:border-b-0 dark:border-gray-800"><td class="px-5 py-4"><div><p class="font-medium text-gray-800 dark:text-white/90">{{ $villa->name }}</p><p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $villa->slug }}</p></div></td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $villa->location ?: 'Belum diisi' }}</td><td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $villa->capacity }}</td><td class="px-5 py-4">@php $badgeColor = match($villa->status) { 'active' => 'success', 'inactive' => 'error', default => 'warning', }; $statusLabel = match($villa->status) { 'active' => 'Aktif', 'inactive' => 'Nonaktif', default => 'Draft', }; @endphp <x-ui.badge :color="$badgeColor">{{ $statusLabel }}</x-ui.badge></td><td class="px-5 py-4"><div class="flex items-center justify-end gap-2"><a href="{{ route('villas.edit', $villa) }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">Edit</a><form method="POST" action="{{ route('villas.destroy', $villa) }}" onsubmit="return confirm('Hapus villa ini?');">@csrf @method('DELETE')<button type="submit" class="rounded-lg border border-error-200 px-4 py-2 text-sm font-medium text-error-600 transition hover:bg-error-50 dark:border-error-800 dark:text-error-400 dark:hover:bg-error-500/10">Hapus</button></form></div></td></tr>
                        @empty
                            <tr><td colspan="5" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada villa yang dibuat.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
