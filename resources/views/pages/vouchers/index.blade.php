...
@section('content')
    ...
    <div class="space-y-6" data-async-page="true">
        ...
        <x-common.component-card title="Filter Voucher" desc="Cari voucher berdasarkan kode, tipe diskon, dan status aktif.">
            <form data-async-page-form="true" method="GET" action="{{ route('vouchers.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div class="xl:col-span-2"><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Pencarian</label><input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Kode voucher" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90" /></div>
                            ...
                            <tr>
                                <td colspan="5" class="px-5 py-10">
                                    <x-common.empty-state
                                        ...
                                        description="Voucher belum wajib di awal, tapi kamu bisa menambahkannya sekarang kalau ingin menyiapkan promo atau diskon booking."
                                        actionLabel="Tambah Voucher"
                                        :actionHref="route('vouchers.create')"
                                    />
                                </td>
                            </tr>
                        @endforelse
