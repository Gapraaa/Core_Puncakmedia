    ...
    <div class="col-span-12 xl:col-span-8">
        <x-common.component-card :title="$pageTitle" :desc="$pageDescription">
            <form method="POST" action="{{ $formAction }}" class="space-y-6"
                data-toast-loading="Data voucher sedang disimpan."
                data-toast-loading-title="{{ isset($httpMethod) ? 'Memperbarui Voucher' : 'Menyimpan Voucher' }}">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                    ...
                    <div><label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Minimum Transaksi</label><input type="number" step="1" min="0" name="minimum_transaction" value="{{ old('minimum_transaction', $voucher->minimum_transaction ?? 0) }}" data-money class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90" /></div>
